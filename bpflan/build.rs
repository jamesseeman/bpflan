...
/// This crate has a runtime dependency on artifacts produced by the `bpflan-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "bpflan-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `bpflan-ebpf`, so cargo will automatically rebuild us
        // if `bpflan-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `bpflan-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("bpflan-ebpf");
